...
def feed():
    ...
    if request.args.get('mode') == 'invalid':
        urls[0] = 'p/THIS_POST_DOES_NOT_EXIST_12345'
...
' | Width: '+innerWidth+' | Page overflow: '+(document.documentElement.scrollWidth>innerWidth);},300);</script></html>''')
    if request.args.get('mode') == 'blocked':
        response.headers['Content-Security-Policy'] = "script-src 'self' 'unsafe-inline'; frame-src 'none'"
    return response
