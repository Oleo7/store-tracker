(function () {
  ...
  function metricDefinitionText(definition = {}) {
    ...
    return `${copy} Täljaren är ${definition.numerator_label} och nämnaren är ${definition.denominator_label}.`;
  }
  ...
  function updateMoreFiltersControl() {
    ...
    label.textContent = activeCount ? `Fler filter · ${activeCount} aktiva` : "Fler filter";
  ...
  function teamComparisonMarkup(team) {
    ...
    const rows = sellers.map(item => `<tr${sellerSelected(item.seller) ? ' class="is-selected"' : ""}><th><button type="button" data-seller="${escapeHtml(item.seller)}">${escapeHtml(item.seller)}</button></th><td data-metric-definition="human_activities">${number(item.human_activities_total)}</td><td data-metric-definition="reach">${rateCell(item.reach)}</td><td data-metric-definition="positive_dialogue">${rateCell(item.positive_dialogue)}</td><td data-metric-definition="positive_to_order_10d">${rateCell(item.positive_to_order_10d, item.waiting_positive_dialogues_count)}</td><td data-metric-definition="order_10d">${rateCell(item.order_10d, item.waiting_outcome_count)}</td><td data-metric-definition="positive_next_step_coverage">${rateCell(item.positive_next_step_coverage)}</td><td data-metric-definition="bom_ratio">${rateCell(item.bom_ratio)}</td></tr>`).join("");
    return `<section class="sc-section" aria-labelledby="sc-team-title"><div class="sc-section-heading"><div><h2 id="sc-team-title">Teamjämförelse</h2><p>10-dagarsmåtten använder samma definition som i Coachningsöversikten. Kontakter vars 10-dagarsfönster fortfarande är öppet ingår i nämnaren, därför markeras måttet som preliminärt när utfall fortfarande väntar. Jämförelsen gäller alla kanaler under vald period, lifecycle och segment; kanal- och säljarfilter påverkar inte teamblocken.</p></div></div><article class="sc-team-chart"><h3 data-metric-definition="human_activities">Mänskliga aktiviteter</h3><p>Besök är stackade: <span class="sc-legend-key is-visit" data-metric-definition="reached_visits">nådda besök</span> + <span class="sc-legend-key is-bom" data-metric-definition="bom_visits">bom</span>. <span class="sc-legend-key is-phone" data-metric-definition="phone">Telefon</span> och <span data-metric-definition="manual_email">manuellt mejl</span> visas separat.</p><div class="sc-team-plot">${activityGroups}</div></article><div class="sc-table-wrap"><table class="sc-table sc-comparison-table"><thead><tr><th>Säljare</th><th>${metricHeader("Aktiviteter", "human_activities", "team-activities")}</th><th>${metricHeader("Träffgrad", "reach", "team-reach")}</th><th>${metricHeader("Positiv dialog", "positive_dialogue", "team-positive")}</th><th>${metricHeader(metricLabel("positive_to_order_10d", "Positiv dialog → order inom 10 dagar"), "positive_to_order_10d", "team-positive-order")}</th><th>${metricHeader(metricLabel("order_10d", "Kontakt – order inom 10 dagar"), "order_10d", "team-order")}</th><th>${metricHeader("Nästa-steg-täckning", "positive_next_step_coverage", "team-next-step")}</th><th>${metricHeader("Bom-ratio", "bom_ratio", "team-bom")}</th></tr></thead><tbody>${rows}</tbody></table></div></section>`;
  }
